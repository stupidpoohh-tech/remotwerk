...
(function (root) {
  ...
  function buildTracks(anim, boneNames) {
  ...
  function mount(container, opts) {
    ...
    for (const bone of skeleton.bones) {
      ...
      if (bone.z != null) el.style.zIndex = String(bone.z);
      ...
      if (bone.part) {
        ...
        part.style.width = P.w + 'px';
        part.style.height = P.h + 'px';
        if (P.shape === 'ellipse') part.style.borderRadius = '50%';
        applySlotStyle(part, rig.slots && rig.slots[P.slot], P);
        el.appendChild(part);
      }
    ...
    function applyPose(pose) {
      ...
      for (const bone of skeleton.bones) {
        ...
        const b = pose.bones[bone.name];
        if (!b) {
          el.style.transform = '';
          continue;
        }
        el.style.transform = `translate(${b.x || 0}px, ${b.y || 0}px) rotate(${b.rot || 0}deg)`;
        el.style.opacity = b.vis === false ? '0' : '1';
      }
    }
    ...
    function playOne(anim, onDone) {
      const built = buildTracks(anim, boneNames);
