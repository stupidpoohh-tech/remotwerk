 ...
 *   4개에서 3개로 더 줄인 것도 같은 이유다 — '보고싶어'(살짝 폴짝)와 '신나'(크게 점프)는
 *   둘 다 위로 뛰는 동작이라, 곁눈질로는 크기 차이만 남아 구분되지 않았다.
 *
 * ▣ 무엇을 남겼나 — 세 방향
 *   각 신호는 **움직임 방향**과 **이펙트 색**으로 이중 구분된다. 곁눈질로도 갈린다.
 *     신나     : 위로 크게 반복 점프  + 노랑 반짝
 *     지쳤어   : 아래로 푹 눌림       + 파랑 땀방울
 *     궁디댄스 : 뒤돌아 좌우로 흔들   + 이펙트 없음(뒤돈다는 것 자체가 신호다)
 *
 *   빠진 '보고싶어'(g_heart)는 RETIRED 에 이름만 남긴다. 예전에 주고받은 신호가
 *   히스토리에 남아 있어서, 지우면 그 줄이 알 수 없는 id 로 보인다.
 *
...
(function (root) {
  ...
  const ACTIVE = [
    ...
    { id: 'g_droop', name: '지쳤어',   icon: '🫠', hint: '축 처져 눌림' },
