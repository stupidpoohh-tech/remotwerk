...
>
> ⚠️ 현재 저장소의 `turn_back/frame-01`·`turn_front/frame-01` 은 여전히 **가짜다.**
> 3/4 측면을 그릴 방법이 없어서 그림을 가로로 얇게 눌러 넣었다. 회전하는 부피가 아니라
> 납작해지는 종이로 읽힌다. 이번 발주에서 **우선순위가 가장 높은 그림**이다.
>
> ✅ 다만 **끝 자세는 고쳤다.** `turn_back/frame-02` 와 `turn_front/frame-00` 을
> 트월킹 원화의 첫 장(`twerk_loop/frame-00`)과 **같은 그림**으로 바꿨다. 예전에는
> 뒤돌기가 5조각 리그로 구운 임시 뒷모습에서 끝나는데 춤은 새 원화로 시작해서,
> 그 이음매에서 팔·머리·명암이 통째로 바뀌며 크게 튀었다. 이제 그 튐은 없고,
> 남은 튐은 `frame-00 → frame-01`(정면 → 눌린 그림) 한 군데뿐이다.
> 가짜인 `frame-01` 을 눌린 그림으로 둔 것도 의도다 — 스타일이 바뀌는 순간을
> 가장 알아보기 어려운 프레임에 몰아넣은 것이다.
...
> | 파일 | 자세 | 유지 |
> |---|---|---|
> | `frame-00` | 중앙 준비 (뒤돌아 서서 어깨 너머로 힐끗) | 120ms |
> | `frame-01` | 왼쪽 궁디 팝 | 200ms |
> | `frame-02` | 중앙 통과 | 120ms |
> | `frame-03` | 오른쪽 궁디 팝 | 200ms |
>
> 한 바퀴 **640ms**, 신호 한 번에 3바퀴. 정점(01·03)을 길게, 통과(00·02)를 짧게 둬서
> 팝이 박자로 읽히게 했다. 네 장 모두 발바닥 y=469~470 으로 접지가 맞고,
> 연속성 검사에서 트월킹 구간 가로변화 1px·발밑변화 1px 이다(발이 미끄러지지 않는다).
>
> **⚠ 측정: 골반 좌우 진폭이 발주 기준의 3분의 1 수준이다.**
> 네 장의 골반 x 를 발 중심 기준으로 재면 좌우 폭이 **19px(키의 5.7%, 한쪽 약 2.8%)** 다.
> 아래 수치 기준은 한쪽 8~10% 였다. 즉 눈에 보이긴 하지만 "확 밀리는" 팝은 아니다.
> 이건 배치로 못 고친다 — 프레임을 좌우로 더 밀면 발까지 같이 움직여 미끄러진다.
> 더 크게 하려면 **골반을 더 밀어 그린 원화**가 필요하다.
>
> 아직 없는 것: 압축·반동 사이 그림(발주표의 02·04·06·08 자리). 지금은 팝에서
> 팝으로 바로 넘어가므로, 곁눈질로는 읽히지만 탄성은 덜하다.

> **왜 예전 임시본으로는 안 됐는가 (측정 결과)**
>
...
>   `source: "import"` 인 클립은 건너뛰도록 막아 뒀다(`--force` 로만 덮어쓴다).
>   막기 전에는 이 도구를 한 번 더 돌리는 것만으로 원화가 임시본으로 되돌아갔다.
>   실제로 가드를 잠시 풀고 돌려 보니 turn_back·turn_front·twerk_loop 이
>   전부 임시본으로 덮어써지는 것을 확인했다.
